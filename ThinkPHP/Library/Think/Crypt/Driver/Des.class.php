      ...
      while ($m < $len) {
        ...
        for ($j=0; $j<$iterations; $j+=3) {
          ...
          for ($i=$looping[$j]; $i!=$endloop; $i+=$loopinc) { //for efficiency
            ...
            //the result is attained by passing these bytes through the Slist selection functions
            $temp = $left;
            $left = $right;
    ...
    private static function _createKeys ($key) {
      ...
      for ($j=0; $j<$iterations; $j++) { //either 1 or 3 iterations
        ...
        for ($i=0; $i < count($shifts); $i++) {
          ...
          //how the Slist selection functions will be applied: S2, S4, S6, S8, S1, S3, S5, S7
